...
interface IResultItemProps {
  item: ISalesFunnelItem;
}

const ResultItemRow: React.FC<IResultItemProps> = ({ item }) => (
    ...
    <div className={styles.resultContent}>
      ...
      {item.summary && (
        <div
          ...
          // summary is already sanitized in the service
...
interface IResultsListProps {
  items: ISalesFunnelItem[];
  emptyMessage: string;
}

const ResultsList: React.FC<IResultsListProps> = ({ items, emptyMessage }) => {
  if (items.length === 0) {
    return (
...
export const SalesFunnelSearch: React.FC<ISalesFunnelSearchProps> = (props) => {
  const { context, displayMode, searchScope, scopeUrl } = props;

  // ------ local state ------
  const [searchText, setSearchText] = useState<string>('');
  // which content types the user wants this search to include
  const [includeDocuments, setIncludeDocuments] = useState<boolean>(
    displayMode !== 'listItems'
  );
  const [includeListItems, setIncludeListItems] = useState<boolean>(
  ...
  // Derive effective display mode from checkboxes
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      const service = new SalesFunnelSearchService(context);
  ...
  if (!scopeUrl) {
    return (
      <div className={styles.configRequired}>
        ...
        <p>
          Open the web part properties panel and provide a Site URL or List/Library URL
          to enable search.
        </p>
      </div>
    );
  ...
  const allItems: ISalesFunnelItem[] = results
    ? [...(results.documents || []), ...(results.listItems || [])]
    : [];

  const visibleItems: ISalesFunnelItem[] =
  ...
  return (
    <div className={styles.salesFunnelSearch}>
      ...
      <div className={styles.searchBar}>
        ...
        <div className={styles.checkboxGroup}>
          {/* Only show checkboxes that the property pane allows */}
          {displayMode !== 'listItems' && (
            <label>
              <input
                type="checkbox"
      ...
      {showTabs && results && (
        <div className={styles.tabContainer} role="tablist">
          ...
          <button
            role="tab"
            aria-selected={activeTab === 'listItems'}
...
export default SalesFunnelSearch;
